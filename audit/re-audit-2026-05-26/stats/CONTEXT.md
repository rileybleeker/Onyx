...
3. **Cross-fitting in AIPW.** Reviewer should verify the split strategy is appropriate for time-series data. Random folds are common in AIPW practice but defensible only if treatment and outcome are conditionally independent given covariates within-fold. With autocorrelation in HRV, this is not obvious.
...
5. **PI coverage.** Reviewer should verify how the std used for prediction intervals is computed (training residuals would systematically under-cover; OOF residuals would better match nominal). Empirical coverage is logged in `hrv_model_metrics.ci_coverage` — compare against the nominal `±1.645σ → 90%` target.
