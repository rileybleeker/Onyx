...
Six FK constraints currently exist (see `SCHEMA_DDL.md` "Referential integrity"). Coverage is selective: the WHOOP cycle hub, Garmin laps → activities, Spotify plays → tracks/artists, and supplement intake → products are FK-enforced. Relationships NOT enforced by FK (a partial list):

- `whoop_workouts.cycle_id → whoop_cycles.cycle_id` — *table has no cycle_id column; WHOOP's API doesn't supply one*
- `habit_journal.question ↔ habit_name_map.name` — text-keyed, loose join in app code
- `hrv_model_metrics.input_data_hash ↔ hrv_predictions.input_data_hash` — cross-reference left implicit

Reviewer should consider whether the absent FKs are deliberate omissions, technical debt, or genuine "should-be-added" candidates given the post-fix integrity surface.
