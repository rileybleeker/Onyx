...
5. **Old `calendar_date` column eventual retirement** — only after every consumer in D5 is verified on the new keys. Probably 1–2 quarters out; not a Phase 4 commitment, just a forward note. **AUDIT 2026-05-25**: every active runtime consumer of `pds.daily_health_matrix` has already migrated to `pds.daily_health_matrix_behavioral`. Confirmed via repo-wide grep: zero matches in `hrv_analysis.py`, `hrv_predict.py`, `causal_inference.py`, `frontend/src/lib/queries.ts`, `frontend/src/app/analytics/hrv/page.tsx`, or any GitHub Actions workflow. The two remaining literal references are (a) `sql/adr_0001_07_snapshot_daily_health_matrix.sql:24` (by design — creates the legacy snapshot) and (b) `supplement_schema.sql:137` (in a comment). The legacy view is therefore effectively orphaned but intentionally retained for ad-hoc spot-checking and as the source for the legacy snapshot. Dropping it is now a low-risk one-line `DROP VIEW pds.daily_health_matrix` whenever Riley wants — recommend waiting through one more weekly cycle of normal Onyx use to surface any forgotten downstream before pulling the trigger.
