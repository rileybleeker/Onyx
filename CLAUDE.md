...
- **HRV variable-coverage audit (2026-05-21)** — `docs/hrv_variable_coverage_audit_2026-05-21.md` enumerates every variable from every data source vs every test (Spearman, Welch journal/habit/supplement/nutrition, Granger, XGBoost/SHAP, SARIMAX/Prophet, AIPW/PSM causal). Fixes applied:
  ...
  - **BH-FDR added to the causal layer (2026-05-27).** `causal_inference.py` was the last uncorrected test family — ~150 AIPW estimates per run at α=0.05 expected ~7-8 false-positive interventions by chance. The fix derives `p_raw` from the AIPW Wald `|ate/se_if|` vs N(0,1), pools binary + continuous treatments into one family of ~150, applies `multipletests(method='fdr_bh')` at `FDR_Q_THRESHOLD=0.05` (shared with `hrv_analysis`), and sets `p_raw`, `p_fdr_adjusted`, `passes_fdr` per result row. `meta_payload` records the correction policy. The descriptive `significant` (CI-excludes-zero) flag is preserved as a separate dimension. First validated run: 14 of 108 treatments survive FDR.
  - **SARIMAX fits on a contiguous daily index (2026-05-27).** Previously `train_sarimax` called `df.dropna(subset=[TARGET]).set_index('calendar_date')`, leaving a row-ordered series with gaps where HRV was missing — SARIMAX(1,1,1)×(1,0,1,7) then treated consecutive rows as consecutive days and biased AR(1) toward 0 (synthetic verification: dropna ar.L1=0.377 vs asfreq 0.494 vs truth 0.500). The fix reindexes to `pd.date_range(first_valid_hrv_day, last_valid_hrv_day, freq='D')` so the Kalman filter integrates over NaN gaps under `enforce_stationarity=False`/`enforce_invertibility=False`. The spine is **clamped to the observed HRV span** (not the full matrix min..max) so years of pre-monitoring NaN don't get injected. Per-horizon log is defensive: when `compute_metrics` returns `{}` (fewer than 3 valid prediction pairs) the horizon is logged as a (n_pairs, n_nan) skip and dropped from `sarimax_metrics`. Stage 3 OLS HAC at `~hrv_analysis.py:1903` cannot reindex (sm.OLS doesn't accept NaN) — the accepted compromise per the audit ticket: pass `cov_kwds={'maxlags': 7, 'use_correction': True}` (Andrews 1991 small-sample correction) and document in-comment that lag-7 here is "rows back" not "calendar days back".
...
- **`pds.caffeine_timing_daily` exposes caffeine intake timing per behavioral day** for the HRV pipeline. Same shape as `pds.meal_timing_daily`: filters `pds.supplement_intake` to rows with a non-null `intake_time` whose product contains caffeine (matched by FDA UNII `3G6A5W338E` or `ingredient_group`/`name ILIKE '%caffeine%'`), groups by behavioral day, and exposes `first_caffeine_hour` + `last_caffeine_hour` (ET decimal hour), `caffeine_window_hours`, `caffeine_intake_count`, plus the bedtime-anchored `last_caffeine_to_bedtime_minutes` (joined via the standard `+12h ET` WHOOP-cycle rule). The bedtime-anchored gap is what HRV reads — monotonic across midnight, unlike the raw clock hour which wraps to 0 (a 12:30 AM pre-bed dose numerically reads as 0.5, inverting the "later caffeine = worse HRV" relationship). The view is LEFT JOINed into `pds.daily_health_matrix_behavioral` exposing `caffeine_first_hour`, `caffeine_last_hour`, `caffeine_window_hours`, `caffeine_intake_count`, and `caffeine_to_bedtime_min`. `caffeine_` is in `CONTROLLABLE_FEATURE_PREFIXES` (so SHAP ranks it as actionable) and all five columns are in `CONTINUOUS_TREATMENTS` in `causal_inference.py` with family `nutrition` (same lifestyle-clustering confounder set the meal-timing layer uses). Intakes logged without a timestamp (`intake_time IS NULL` — "took it today, time unspecified") are deliberately excluded — they'd corrupt the timing aggregate. Coverage activates automatically once ~18 days clear the 5% non-null filter; AIPW estimates need ≥10 days per median-split arm. Schema in `sql/caffeine_timing_daily.sql`.
- **`/nutrition` meal quick-log exposes both `last_meal` and `first_meal` kinds.** A two-pill row (Kind: last meal / first meal) above the time selector toggles `mealKind` in component state and is sent as `kind` in the `POST /api/meals` body. Defaults to `last_meal` to preserve the prior single-kind UX; the button label flips to "Log first meal" when first meal is selected. The behavioral-day attribution logic (post-midnight auto-yesterday) applies identically to both kinds — a 6 AM first meal that's the day's start lands on today, a 12:30 AM pre-bed last meal lands on yesterday, as before. `first_meal` events feed `meal_first_hour` and `meal_eating_window_hours` in the matrix, both of which were already wired through the HRV pipeline (Spearman / SHAP / causal) and just lacked an ergonomic logging surface.
...
Production tripwire for the class of bug `tsc` + `lint` cannot catch: a panel that **should** have data silently falling back to its empty state. The Spotify FK-ordering, HRV `JSON.parse`-of-jsonb, and supplements renamed-column regressions all shipped green and only surfaced when Riley clicked around production — each as an empty/broken panel.

- **Core principle:** *empty-state fallback present where data should exist = test failure.*
- **What it covers:** loads every dashboard page against **real production data** (authenticated) and asserts (a) the page isn't bounced to `/login` and doesn't hit the Next.js client error boundary, (b) a real data element rendered (recharts data marks / table rows / journal list items), and (c) **known empty-state fallback strings are ABSENT**. Two pages get precise assertions: `/analytics/hrv` (HRV Correlates draws bars + none of the "…not yet computed" / "Run hrv_analysis.py to compute correlations" fallbacks) and `/supplements` (the "Today's compound totals" table is populated whenever `/api/supplements/today` reports intakes — the test self-skips on a legitimately empty day rather than false-fail). Strings that can be legitimately empty (ReccoBeats-thin Spotify subsets, manual-entry meal/weight sections, range-windowed lists) are intentionally **not** asserted.
- **Files:** `frontend/playwright.config.ts` (single chromium project, `retries:1`, reuses a saved `storageState`), `frontend/e2e/auth.setup.ts` (logs in once via `/login`, saves `e2e/.auth/state.json` — gitignored), `frontend/e2e/smoke.spec.ts`. The `e2e/` dir + `playwright.config.ts` are excluded from `tsconfig.json` so they don't affect `next build`.
- **Run locally:** `cd frontend && npx playwright test` (add `--ui` to watch / debug). Defaults to the prod Vercel URL via `SMOKE_BASE_URL`.
- **Required env** (3 vars) — read from `frontend/.env.local` locally and from **GitHub repo secrets** in CI: `SMOKE_TEST_EMAIL`, `SMOKE_TEST_PASSWORD`, `SMOKE_BASE_URL`. (The login uses Supabase Auth email+password; use a real app account.)
- **CI:** `.github/workflows/smoke-test.yml` runs every 6h (`0 */6 * * *`) + `workflow_dispatch`. A failure = red X + GitHub's failure email (the alert); the Playwright HTML report uploads as an artifact on failure.
- **Adding a page:** append to the `PAGES` array (or add a dedicated `test.describe`) in `smoke.spec.ts`. Prefer text-absence of the real empty-state string + a "some data element exists" check over brittle per-element selectors.
